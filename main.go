...
func main() {
	...
	for _, file := range filesList {
		if strings.Contains(file[1].(string), "RQProfile.sav") {
			if oldFile != nil {
	...
	for i, file := range filesList {
		if file[0] == oldFile[0] {
			filesList[i][1] = "RQProfileCopy.sav"
		}
	}
